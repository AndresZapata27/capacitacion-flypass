package com.courses.capacitacionesflypass.sesion1.f.solid.LiskovSubstitution.casoDos;
...
import com.jamiltonquintero.solid.l.sinl.casoDos.model.User;
import com.jamiltonquintero.solid.l.sinl.casoDos.model.UserRepository;
...
@Service
public class ReporteAuditoria implements IReporte {
    private final UserRepository userRepository;

    public ReporteAuditoria(UserRepository userRepository) {
    ...
    @Override
    public void conclusion(Document document) throws DocumentException {
    ...
    @Override
    public void totalCredito(Document document) throws DocumentException {
        throw new RuntimeException("Metodo no soportado para auditoria");
    }

    @Override
    public void auditoria(Document document) throws DocumentException {

        Font conclusion = FontFactory.getFont(FontFactory.HELVETICA_BOLD, 60, BaseColor.BLACK);

        Paragraph titleParagraph = new Paragraph("Reporte para auditoria" , conclusion);
        titleParagraph.setAlignment(Element.ALIGN_CENTER);
        titleParagraph.setSpacingBefore(document.getPageSize().getHeight() / 2 - conclusion.getSize());
        document.add(titleParagraph);
    }
